...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		ImGui::Begin("bezier");
		ImGui::SliderFloat3("controlPoint0", &controlPoints[0].x, -10.0f, 10.0f);
		ImGui::SliderFloat3("controlPoint1", &controlPoints[1].x, -10.0f, 10.0f);
		ImGui::SliderFloat3("controlPoint2", &controlPoints[2].x, -10.0f, 10.0f);
		ImGui::End();

		Sphere controlPointsSphere[3] = {
		{controlPoints[0],0.01f,BLACK},
		{controlPoints[1],0.01f,BLACK},
		{controlPoints[2],0.01f,BLACK},
		};
		...
		for (int i = 0; i < 3; ++i) {
			DrawSphere(controlPointsSphere[i], viewProjectionMatrix, viewportMatrix, controlPointsSphere[i].color);
		}
...
void DrawBezier(const Vector3& controlPoint0, const Vector3& controlPoint1, const Vector3& controlPoint2, Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewportMatrix, uint32_t color)
{
	for (int i = 0; i < 32; ++i) {
		...
		Vector3 Bezier01 = Lerp(controlPoint0, controlPoint1, t);
		Vector3 Bezier12 = Lerp(controlPoint1, controlPoint2, t);

		Vector3 startBezier = Lerp(Bezier01, Bezier12, t);
		Vector3 endBezier = Lerp(Bezier01, Bezier12, t + (1.0f / 16.0f));

		Matrix4x4 startWorldMatrix = MakeAffineMatrix({ 1.0f,1.0f,1.0f }, { 0.0f,0.0f,0.0f }, startBezier);
		Matrix4x4 endWorldMatrix = MakeAffineMatrix({ 1.0f,1.0f,1.0f }, { 0.0f,0.0f,0.0f }, endBezier);
